import * as A from "@effect/schema-3/Arbitrary"
...
describe("JSONSchema", () => {
  ...
  it("a unique symbol should raise an error", () => {
    expect(() => JSONSchema.to(Schema.uniqueSymbol(Symbol.for("@effect/schema-3/test/a")))).toThrow(
  ...
  describe("struct", () => {
    ...
    it("should raise an error if there is a property named with a symbol", () => {
      ...
      expect(() => JSONSchema.to(Schema.struct({ [a]: Schema.string }))).toThrow(
        new Error("Cannot encode Symbol(@effect/schema-3/test/a) key to JSON Schema")
      )
    })
  })
