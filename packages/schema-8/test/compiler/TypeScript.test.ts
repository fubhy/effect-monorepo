...
describe("TypeScript", () => {
  ...
  it("uniqueSymbol", () => {
    const schema = S.uniqueSymbol(Symbol.for("@effect/schema-8/test/a"), {
      [AST.IdentifierAnnotationId]: "a"
    })
    const ts = typeScriptFor(schema)
    expect(printNodes(ts.nodes)).toEqual([`a = Symbol.for("@effect/schema-8/test/a")`, `typeof a`])
  })
  ...
  describe("tuple", () => {
    ...
    it("all with symbols", () => {
      const schema = S.tuple(
        S.uniqueSymbol(Symbol.for("@effect/schema-8/test/a"), {
          [AST.IdentifierAnnotationId]: "a"
        })
      ).pipe(
        S.rest(S.uniqueSymbol(Symbol.for("@effect/schema-8/test/b"), {
          [AST.IdentifierAnnotationId]: "b"
        })),
        S.element(S.uniqueSymbol(Symbol.for("@effect/schema-8/test/c"), {
          [AST.IdentifierAnnotationId]: "c"
        }))
      )
      const ts = typeScriptFor(schema)
      expect(printNodes(ts.nodes)).toEqual([
        `a = Symbol.for("@effect/schema-8/test/a")`,
        `b = Symbol.for("@effect/schema-8/test/b")`,
        `c = Symbol.for("@effect/schema-8/test/c")`,
        `readonly [
    typeof a,
    ...(typeof b)[],
    typeof c
  ...
  describe("struct", () => {
    ...
    it("all with symbols", () => {
      const a = Symbol.for("@effect/schema-8/test/a")
      const b = Symbol.for("@effect/schema-8/test/b")
      const schema = S.struct({
        ...
        S.extend(
          S.record(
            S.string,
            S.uniqueSymbol(Symbol.for("@effect/schema-8/test/d"), {
      ...
      expect(printNodes(ts.nodes)).toEqual([
        `b = Symbol.for("@effect/schema-8/test/b")`,
        `d = Symbol.for("@effect/schema-8/test/d")`,
        `{
    readonly [Symbol.for("@effect/schema-8/test/a")]: typeof b;
    readonly c: number;
