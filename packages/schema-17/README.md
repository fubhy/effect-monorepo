...
<p align="center">
  <a href="https://www.npmjs.com/package/@effect/schema-17">
    <img src="https://img.shields.io/npm/dm/@effect/schema-17.svg" alt="npm downloads" height="20">
  </a>
</p>

# Introduction

Welcome to the documentation for `@effect/schema-17`, **a library for defining and using schemas** to validate and transform data in TypeScript.

`@effect/schema-17` allows you to define a `Schema<I, A>` that provides a blueprint for describing the structure and data types of your data. Once defined, you can leverage this schema to perform a range of operations, including:
...
```ts
import * as Schema from "@effect/schema-17/Schema";
...
If, for some reason, you can't enable the `exactOptionalPropertyTypes` option (perhaps due to conflicts with other third-party libraries), you can still use `@effect/schema-17`. However, there will be a mismatch between the types and the runtime behavior:
...
Once you have installed the library, you can import the necessary types and functions from the `@effect/schema-17/Schema` module.
...
In addition to the provided `struct` and `union` functions, `@effect/schema-17/Schema` also provides a number of other functions for defining `Schema`s, including functions for defining arrays, tuples, and records.
...
To parse a value from the `unknown` type using the previously defined `Schema`, you can make use of the `parse*` functions provided by the `@effect/schema-17/Schema` module. Let's explore an example using the `parseEither` function:
...
```ts
import * as S from "@effect/schema-17/Schema";
import { formatErrors } from "@effect/schema-17/TreeFormatter";
import * as Either from "effect/Either";
...
```ts
import { formatErrors } from "@effect/schema-17/ArrayFormatter";
import * as S from "@effect/schema-17/Schema";
import * as Either from "effect/Either";
...
The `is` function provided by the `@effect/schema-17/Schema` module represents a way of verifying that a value conforms to a given `Schema`. `is` is a refinement that takes a value of type `unknown` as an argument and returns a `boolean` indicating whether or not the value conforms to the `Schema`.
...
The `arbitrary` function provided by the `@effect/schema-17/Arbitrary` module represents a way of generating random values that conform to a given `Schema`. This can be useful for testing purposes, as it allows you to generate random test data that is guaranteed to be valid according to the `Schema`.

```ts
import * as Arbitrary from "@effect/schema-17/Arbitrary";
import * as S from "@effect/schema-17/Schema";
...
The `to` function provided by the `@effect/schema-17/Pretty` module represents a way of pretty-printing values that conform to a given `Schema`.
...
The `to` / `from` functions, which are part of the `@effect/schema-17/JSONSchema` module, allow you to generate a JSON Schema based on a schema definition:

```ts
import * as JSONSchema from "@effect/schema-17/JSONSchema";
import * as S from "@effect/schema-17/Schema";
...
The `to` function, which is part of the `@effect/schema-17/Equivalence` module, allows you to generate an [Equivalence](https://effect-ts.github.io/effect/modules/Equivalence.ts.html) based on a schema definition:

```ts
import * as S from "@effect/schema-17/Schema";
import * as Equivalence from "@effect/schema-17/Equivalence";
...
In the `@effect/schema-17/Schema` library, you can apply custom validation logic using _filters_.
...
```ts
import * as ParseResult from "@effect/schema-17/ParseResult";
import * as S from "@effect/schema-17/Schema";
...
To define a schema for a branded type from scratch, you can use the `brand` combinator exported by the `@effect/schema-17/Schema` module. Here's an example:
...
If you have already defined a branded type using the `effect/Brand` module, you can reuse it to define a schema using the `fromBrand` combinator exported by the `@effect/schema-17/Schema` module. Here's an example:
...
`@effect/schema-17/Schema` includes a built-in `union` combinator for composing "OR" types.
...
This code defines a discriminated union using the `@effect/schema-17` library:
...
To perform these kinds of transformations, the `@effect/schema-17` library provides the `transform` combinator.
...
import * as ParseResult from "@effect/schema-17/ParseResult";
import * as S from "@effect/schema-17/Schema";
import * as TreeFormatter from "@effect/schema-17/TreeFormatter";
import * as Effect from "effect/Effect";
...
The `optionFromNullable` combinator in `@effect/schema-17/Schema` allows you to specify that a field in a schema is of type `Option<A>` and can be parsed from a required nullable field `A | null`. This is particularly useful when working with JSON data that may contain `null` values for optional fields.
...
Then we can implement the body using the APIs exported by the `@effect/schema-17/AST` module:

```ts
import * as S from "@effect/schema-17/Schema";
import * as AST from "@effect/schema-17/AST";
import * as Option from "effect/Option";
...
One of the fundamental requirements in the design of `@effect/schema-17` is that it is extensible and customizable. Customizations are achieved through "annotations". Each node contained in the AST of `@effect/schema-17/AST` contains an `annotations: Record<symbol, unknown>` field that can be used to attach additional information to the schema.
...
Output:
{
  ast: {
    _tag: 'StringKeyword',
    annotations: {
      [Symbol(@effect/schema-17/annotation/Title)]: 'string',
      [Symbol(@effect/schema-17/annotation/Description)]: 'a string',
