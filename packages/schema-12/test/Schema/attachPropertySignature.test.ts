...
describe("Schema/attachPropertySignature", () => {
  ...
  it("symbol keys + literal values", async () => {
    const Circle = S.struct({ radius: S.number })
    const Square = S.struct({ sideLength: S.number })
    const kind = Symbol.for("@effect/schema-12/test/kind")
    const schema = S.union(
      Circle.pipe(S.attachPropertySignature(kind, "circle")),
      Square.pipe(S.attachPropertySignature(kind, "square"))
    )
  ...
  it("string keys + unique symbols", async () => {
    ...
    const Square = S.struct({ sideLength: S.number })
    const kind = Symbol.for("@effect/schema-12/test/kind")
    const circle = Symbol.for("@effect/schema-12/test/circle")
    const square = Symbol.for("@effect/schema-12/test/square")
    const schema = S.union(
      Circle.pipe(S.attachPropertySignature(kind, circle)),
      Square.pipe(S.attachPropertySignature(kind, square))
    )
