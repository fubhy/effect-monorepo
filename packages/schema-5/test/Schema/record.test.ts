  ...
  describe("decoding", () => {
    ...
    it("record(string, number)", async () => {
      ...
      await Util.expectParseFailure(
        schema,
        { a: 1, [b]: "b" },
        "/Symbol(@effect/schema-5/test/b) is unexpected, expected string",
    ...
    it("record(symbol, number)", async () => {
      ...
      await Util.expectParseFailure(
        schema,
        { [a]: "a" },
        `/Symbol(@effect/schema-5/test/a) Expected number, actual "a"`
      )
    ...
    it("record(keyof struct({ a, b } & Record<symbol, string>), number)", async () => {
      ...
      await Util.expectParseSuccess(schema, { a: 1, b: 2 })
      const c = Symbol.for("@effect/schema-5/test/c")
      ...
      await Util.expectParseFailure(
        schema,
        { a: 1, b: 2, [c]: "c" },
        `/Symbol(@effect/schema-5/test/c) Expected number, actual "c"`
      )
      await Util.expectParseFailure(schema, { a: "a" }, `/a Expected number, actual "a"`)
    })
    ...
    it("record(Symbol('a') | Symbol('b'), number)", async () => {
      ...
      await Util.expectParseFailure(schema, {}, `/Symbol(@effect/schema-5/test/a) is missing`)
      await Util.expectParseFailure(schema, { [a]: 1 }, `/Symbol(@effect/schema-5/test/b) is missing`)
      await Util.expectParseFailure(schema, { [b]: 2 }, `/Symbol(@effect/schema-5/test/a) is missing`)
    })
